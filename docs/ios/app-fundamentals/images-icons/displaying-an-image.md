...
> [!IMPORTANT]
> **Not**: bir görüntü kümesine atanan görüntüleri doğru görünmüyorsa, doğru dosya adı ile kullanıldığından emin olun `FromBundle` yöntemi ( **resmi ayarlama** ve üst **varlık Kataloğu**  adı). PNG görüntüleri için `.png` uzantısı etmeyebilirsiniz. Diğer resim biçimleri için uzantısı (ör. gereklidir `PurpleMonkey.jpg`).
