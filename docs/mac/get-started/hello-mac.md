...
C# ve .NET aynı macOS geliştirme sırasında kullanılan API'leri kullanarak tamamen yerel Mac uygulamaları geliştirilmesini sağlar Xamarin.Mac *Objective-C* veya *Swift*. Xamarin.Mac Xcode ile doğrudan tümleşir çünkü Geliştirici Xcode'un kullanabilirsiniz _arabirim Oluşturucu_ bir uygulamanın kullanıcı arabirimleri oluşturun (veya isteğe bağlı olarak bunları doğrudan C# kodu oluşturmak için).

Ayrıca, bu yana Xamarin.Mac uygulamaları C# ve .NET dillerinde yazılan kod Xamarin.iOS ve Xamarin.Android mobil uygulamalarla paylaşılabilir; her platformda tüm yerel deneyimi sunarken.
...
[![](hello-mac-images/run02-sml.png "Merhaba, çalıştıran Mac uygulaması örneği")](hello-mac-images/run02.png#lightbox)
...
Uygulama geliştirme Xamarin.Mac gerektirir:
...
- MacOS 10.7 veya üzeri çalıştıran bir Mac bilgisayara.
...
1. Mac için Visual Studio'yu başlatın:
    ...
    [![](hello-mac-images/setup01-sml.png "Ana Visual Studio Mac arabirimi")](hello-mac-images/setup01.png#lightbox)

2. Tıklayarak **yeni proje...**  açmak için düğmeyi **yeni proje** iletişim kutusuna ve ardından **Mac** > **uygulama** > **Cocoauygulaması** tıklatıp **sonraki** düğmesi:

    [![](hello-mac-images/setup02-sml.png "Cocoa uygulaması seçme")](hello-mac-images/setup02.png#lightbox)

3. Girin `Hello_Mac` için **uygulama adı**ve varsayılan her şey tutun. Tıklayın **sonraki**:

    [![](hello-mac-images/setup03-sml.png "Uygulamanın adını ayarlama")](hello-mac-images/setup03.png#lightbox)

4. Yeni Proje bilgisayarınızdaki konumunu doğrulayın:

    [![](hello-mac-images/setup04-sml.png "Yeni çözüm ayrıntılarını doğrulanıyor")](hello-mac-images/setup04.png#lightbox)
...
[![](hello-mac-images/project01-sml.png "Yeni çözüm varsayılan görünümü")](hello-mac-images/project01.png#lightbox)

Mac için Visual Studio kullanan aynı **çözüm** ve **proje** yapısı olarak Visual Studio 2017. Bir çözüm bir veya daha fazla proje tutan bir kapsayıcıdır; projeleri, uygulamaları, kitaplıkları, test uygulamaları destekleyen içerebilir. **Dosya > Yeni proje** şablon bir çözüm ya da bir uygulama projesini otomatik olarak oluşturur.

## <a name="anatomy-of-a-xamarinmac-application"></a>Bir Xamarin.Mac uygulamasını anatomisi

Xamarin.Mac uygulama programlama Xamarin.iOS ile çalışmaya benzer. iOS bir Cocoa Mac tarafından kullanılan, slimmed aşağı sürümüdür CocoaTouch framework kullanır.

Proje dosyalarında göz atın:

- **Main.cs** uygulamanın ana girdi noktası içerir. Uygulama başlatıldığında `Main` sınıfı çalıştırılan çok ilk yöntem içerir.
- **AppDelegate.cs** içeren `AppDelegate` işletim sisteminden olayları dinlemek için sorumlu sınıfı.
- **Info.plist** uygulama adı, simgeler vb. gibi uygulama özellikleri içerir.
- **Entitlements.plist** uygulama yetkilendirmelerini içerir ve korumalı alana alma ve iCloud desteği gibi şeyler erişim sağlar.
...
- **ViewController.cs** ana pencereyi denetleyicisidir. Denetleyicileri başka bir makalede ayrıntılı ele alınacak, ancak şimdilik, ana herhangi bir görünüm altyapısı olan bir denetleyici düşünülebilir.
- **ViewController.designer.cs** yardımcı ana ekran kullanıcı arabirimi ile tümleştirilen tesisat kodunu içerir.
...
`ViewController` Ana pencerenin denetleyicisi bir sınıftır. Denetleyici için yaşam döngüsü ana penceresinin sorumludur. Bu ayrıntılı olarak daha sonra Şimdi Al, hızlı bir bakış için inceleneceğini:
...
Ana pencere sınıfı için tasarımcı dosyası başlangıçta boş olan, ancak kullanıcı arabirimi, Xcode arabirim Oluşturucu ile oluşturulan, otomatik olarak Visual Studio tarafından Mac için doldurulur:
...
Otomatik olarak herhangi bir pencere veya uygulama görünümünde eklenmiş olan denetimlere erişim veren tesisat kod sağlamak üzere Mac için Visual Studio tarafından yönetildikleri gibi Tasarımcısı dosyaları doğrudan düzenlenmemelidir.
...
Ayrıca tanımlar _film şeridi_ altında Xamarin.Mac uygulama için kullanıcı arabirimini görüntülemek için kullanılacak **ana arabirimi** açılır. Yukarıdaki örnekte `Main` açılan listede ilişkili `Main.storyboard` projenin kaynak ağacında **Çözüm Gezgini**. Ayrıca belirterek uygulamanın simgelerinden tanımlar *varlık Kataloğu* bunları içeren (**AppIcon** bu durumda).
...
Bu Xcode başlatın ve bu ekran görüntüsündeki gibi:
...
Açarken bir **.storyboard** dosya ile açılır Xcode'da Mac için Visual Studio'dan bir **Proje Gezgini** soldaki **arabirimi hiyerarşi** ve  **Düzenleyici arabirimi** ortada ve **elektrik ve özellikleri** bölümünde sağ taraftaki:
...
Arabirim Oluşturucu kullanmak için aşağıdaki adımları izleyin:
...
3. Tıklayarak **başlık** özelliğinde **özniteliği denetçisi** düğmenin başlık değiştirip **Me tıklayın**:
...
Yeniden boyutlandırma ve taşıma denetimlerin çalışırken, arabirim Oluşturucu dayalı yararlı ek ipuçları verir dikkat edin [macOS İnsan Arabirimi yönergelerine](https://developer.apple.com/design/human-interface-guidelines/macos/overview/themes/). Bu yönergeleri, bilinen bir görünümü ve deneyimini Mac kullanıcıları için olan yüksek kaliteli uygulamalar oluşturmak için geliştirici yardımcı olur.
...
[![](hello-mac-images/xcode16-sml.png "Xcode kaynağı görüntüleme")](hello-mac-images/xcode16.png#lightbox)
...
6. **Son adım çok önemli!** :, seçilen doğru dosya yoksa, oluşturmanız mümkün olmayacaktır **çıkışlar** ve **eylemleri**, ya da yanlış sınıfa C# dilinde sunulur!
...
[![](hello-mac-images/sync01-sml.png "Xcode ile değişiklikler eşitleniyor")](hello-mac-images/sync01.png#lightbox)
...
[![](hello-mac-images/code01-sml.png "ViewController.cs dosyası, Mac için Visual Studio'da görüntüleme")](hello-mac-images/code01.png#lightbox)
...
Geliştirici derlemeden türünü seçebilirsiniz **yapılandırması Seçici** ekran Mac için Visual Studio sol üst köşesindeki:

[![](hello-mac-images/run01-sml.png "Hata ayıklama derleme seçme")](hello-mac-images/run01.png#lightbox)
...
[![](hello-mac-images/run02-sml.png "Uygulamayı çalıştırma")](hello-mac-images/run02.png#lightbox)

Birkaç kez düğmesine tıkladıysanız, etiket sayısı ile güncelleştirilmesi gereken:

[![](hello-mac-images/run03-sml.png "Düğmeye tıklandığında, sonuçlar gösteriliyor")](hello-mac-images/run03.png#lightbox)
...
- [Menüler](~/mac/user-interface/menu.md) -menüler, çeşitli parçaları bir Mac uygulamanın kullanıcı arabirimini; herhangi bir pencere içinde görünebilir pop up ve bağlamsal menü için ekranın üst uygulamanın ana menüden kullanılır. Menüler Mac uygulamanın kullanıcı deneyimini ayrılmaz bir parçasıdır. Bu makale, bir Xamarin.Mac uygulamasını Cocoa Menülerle çalışma kapsar.
- [Araç çubukları](~/mac/user-interface/toolbar.md) -araç çubukları çalışmak bir Xamarin.Mac uygulamasında bu makalede ele alınmıştır. Oluşturulması ve bakımının yapılması ele alınmaktadır. Araç çubukları Xcode ve arabirim oluşturucu araç çubuğu öğelerini çıkışlar ve eylemleri kullanarak, etkinleştirme ve araç çubuğu öğelerini devre dışı bırakma ve son olarak araç çubuğu öğeleri için C# kodunda yanıt kodu nasıl sunacağınızı öğrenin.
...
[Mac Örnekler Galerisi](https://developer.xamarin.com/samples/mac/all/) Xamarin.Mac edinin yardımcı olmak için kullanıma hazır kod örneklerini içerir.

Birçok tipik bir Mac uygulamasında bulmak için bir kullanıcı beklediğiniz özellikleri içeren bir tam Xamarin.Mac uygulama [SourceWriter örnek uygulaması](https://developer.xamarin.com/samples/mac/SourceWriter/). SourceWriter kod tamamlama ve basit söz dizimi vurgulama için destek sağlayan bir basit kaynak kod düzenleyicidir.

SourceWriter kod tamamen yorum yaptı ve mümkün olan durumlarda bağlantıları anahtar teknolojileri veya yöntemleri Xamarin.Mac belgelerinde ilgili bilgileri verildi.
...
- [macOS İnsan Arabirimi yönergelerine](https://developer.apple.com/design/human-interface-guidelines/macos/overview/themes/)
