...
Her üç `FlexLayout` görünümleri, boş bir `Label` önündeki `Button`, ancak bir `Grow` 1 ayarlayarak. Bu fazladan dikey spae bu boş olarak ayrılan anlamına gelir `Label`, hangi etkili bir şekilde iter `Button` altına.
