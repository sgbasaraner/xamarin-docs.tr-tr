...
[ `VideoPlayer` ](https://github.com/xamarin/xamarin-forms-samples/blob/master/CustomRenderers/VideoPlayerDemos/VideoPlayerDemos/VideoPlayerDemos/VideoPlayer.cs) Sınıftır parçası **VideoPlayerDemos** platformları arasında paylaşılan taşınabilir sınıf kitaplığı (PCL). Öğesinden türetilen `View`:
