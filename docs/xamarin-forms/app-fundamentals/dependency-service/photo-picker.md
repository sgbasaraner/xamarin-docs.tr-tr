...
Yöntem erişir `MainActivity` çeşitli amaçlarla sınıfı: için `PickImageId` için alan `TaskCompletionSource` özelliği ve çağırmak için `StartActivityForResult`. Bu yöntem tarafından tanımlanan `FormsApplicationActivity` sınıf temel sınıfını `MainActivity`.
