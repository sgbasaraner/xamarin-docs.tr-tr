...
1. İçinde **Yeni Öğe Ekle** iletişim kutusunda **Visual C# öğeleri > Xamarin.Forms > içerik sayfasını**, yeni öğe adı **CallHistoryPage**, tıklatıp **Ekle**  düğmesi. Bu adlı bir sayfaya ekler **CallHistoryPage** projeye:

  ![](quickstart-images/vs/add-callhistorypage-class.png "Xamarin.Forms proje şablonları")

1. İçinde **CallHistoryPage.xaml**, tüm şablonu kodu kaldırın ve aşağıdaki kod ile değiştirin. Bu kod sayfası için kullanıcı arabirimi bildirimli olarak tanımlar:
...
1. İçinde **Çözüm Gezgini**, çift **App.xaml.cs** açmak için:

  ![](quickstart-images/vs/open-app-class.png "App.xaml.cs açın")

1. İçinde **App.xaml.cs**, içeri aktarma `System.Collections.Generic` ad alanı, bildirimi ekleme `PhoneNumbers` özelliği, özelliğinde başlatma `App` oluşturucu ve başlatma [ `MainPage` ](https://developer.xamarin.com/api/property/Xamarin.Forms.Application.MainPage/) özelliğini bir [ `NavigationPage` ](https://developer.xamarin.com/api/type/Xamarin.Forms.NavigationPage/). `PhoneNumbers` Koleksiyonu, bir uygulama tarafından aranan her çevrilen telefon numarası listesini depolamak için kullanılacak:
...
1. İçinde **Çözüm Gezgini**, çift **MainPage.xaml** açmak için:

  ![](quickstart-images/vs/open-mainpage-xaml.png "MainPage.xaml açın")

1. İçinde **MainPage.xaml**, ekleme bir [ `Button` ](https://developer.xamarin.com/api/type/Xamarin.Forms.Button/) sonunda denetim [ `StackLayout` ](https://developer.xamarin.com/api/type/Xamarin.Forms.StackLayout/) denetim. Düğme çağrısı geçmişi sayfasına gitmek için kullanılır:
...
1. İçinde **Çözüm Gezgini**, çift **MainPage.xaml.cs** açmak için:

  ![](quickstart-images/vs/open-mainpage-codebehind.png "MainPage.xaml.cs açın")

1. İçinde **MainPage.xaml.cs**, ekleme `OnCallHistory` olay işleyicisi yöntemi ve değiştirme `OnCall` çevrilen telefon numarası eklemek üzere olay işleyicisi yöntemi `App.PhoneNumbers` toplama ve Etkinleştir `callHistoryButton`, koşuluyla `dialer` değişken değil `null`:
...
1. İçinde **Çözüm Gezgini**, sağ tıklayın **Phoneword.Droid** proje ve seçin **başlangıç projesi olarak ayarla**.
...
1. İçinde **yeni dosya** iletişim kutusunda **Forms > Forms ContentPage Xaml**, yeni dosya adı **CallHistoryPage**, tıklatıp **yeni** düğme. Bu adlı bir sayfaya ekler **CallHistoryPage** projeye:

  ![](quickstart-images/xs/add-callhistorypage-class.png "Add Forms ContentPage")

1. İçinde **çözüm paneli**, çift **CallHistoryPage.xaml** açmak için:
