...
description: Bu belge, .NET katıştırma Android ile kullanmaya başlamak açıklar. Bu, .NET bir Android kitaplığıdır projesi oluşturma oluşturulan çıktıda bir Android Studio projesi ve diğer durumlar kullanarak katıştırma, yükleme açıklanır.
ms.prod: xamarin
