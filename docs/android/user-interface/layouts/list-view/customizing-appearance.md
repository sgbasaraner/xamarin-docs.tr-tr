...
- **SimpleListItemSingleChoice** &ndash; burada yalnızca bir seçimdir olası radyo düğmesi türü listeleri oluşturur.
...
`ListView` Kendi görüntülenmekte donatıyı bağımsız olarak farklı seçim modunu destekler. Karışıklığı önlemek için `Single` seçim modu ile `SingleChoice` Donatılar ve `Checked` veya `Multiple` moduyla `MultipleChoice` stili. Seçim modu tarafından denetlenen `ChoiceMode` özelliği `ListView`.
